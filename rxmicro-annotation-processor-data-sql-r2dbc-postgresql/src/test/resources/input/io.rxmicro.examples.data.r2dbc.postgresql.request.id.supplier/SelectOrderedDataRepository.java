...
public interface SelectOrderedDataRepository {
    ...
    CompletableFuture<List<Account>> findAllOrderedById(RequestIdSupplier requestIdSupplier);

    @Select("SELECT * FROM ${table} ORDER BY ( id ? )")
    CompletableFuture<List<Account>> findAllOrderedById(RequestIdSupplier requestIdSupplier, SortOrder sortOrder);

    @Select("SELECT * FROM ${table} ORDER BY ? ?")
    CompletableFuture<List<Account>> findAllOrderedBy(RequestIdSupplier requestIdSupplier, String columnName, SortOrder sortOrder);

    @Select("SELECT * FROM ${table} ORDER BY (id ?, email ?) LIMIT 10")
    CompletableFuture<List<Account>> findAllOrderedByIdAndEmail(
            RequestIdSupplier requestIdSupplier, @RepeatParameter(2) SortOrder sortOrder
    );
}
// end::content[]
