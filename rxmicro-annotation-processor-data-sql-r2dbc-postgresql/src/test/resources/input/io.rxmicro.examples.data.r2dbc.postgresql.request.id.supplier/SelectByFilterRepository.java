...
public interface SelectByFilterRepository {
    ...
    CompletableFuture<List<Account>> findByFirstName(
            RequestIdSupplier requestIdSupplier, String firstName1, String firstName2, String firstName3
    );

    @Select("SELECT * FROM ${table} WHERE balance BETWEEN ? AND ?")
    CompletableFuture<List<Account>> findByBalance(RequestIdSupplier requestIdSupplier, BigDecimal minBalance, BigDecimal maxBalance);

    @Select("SELECT * FROM ${table} WHERE first_name=? OR last_name=?")
    CompletableFuture<List<Account>> findByFirstOrLastName(RequestIdSupplier requestIdSupplier, String name1, String name2);

    @Select("SELECT * FROM ${table} WHERE first_name ILIKE ? OR last_name ILIKE ?")
    CompletableFuture<List<Account>> findByFirstOrLastName(RequestIdSupplier requestIdSupplier, @RepeatParameter(2) String name);
}
// end::content[]
