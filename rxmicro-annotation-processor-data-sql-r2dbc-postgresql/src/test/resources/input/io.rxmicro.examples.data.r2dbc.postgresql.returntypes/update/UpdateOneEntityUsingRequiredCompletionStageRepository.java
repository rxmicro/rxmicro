...
public interface UpdateOneEntityUsingRequiredCompletionStageRepository {
    ...
    CompletionStage<Account> update04(Account account);

    @Update("UPDATE ${table} SET ${updated-columns} WHERE ${by-id-filter} RETURNING *")
    CompletionStage<EntityFieldMap> update05(Account account);

    @Update("UPDATE ${table} SET ${updated-columns} WHERE ${by-id-filter} RETURNING *")
    CompletionStage<EntityFieldList> update06(Account account);
}
