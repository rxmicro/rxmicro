...
public interface DeleteOneEntityUsingMonoRepository {
    ...
    Mono<Account> delete04(Account account);

    @Delete("DELETE FROM ${table} WHERE ${by-id-filter} RETURNING *")
    Mono<EntityFieldMap> delete05(Account account);

    @Delete("DELETE FROM ${table} WHERE ${by-id-filter} RETURNING *")
    Mono<EntityFieldList> delete06(Account account);
}
