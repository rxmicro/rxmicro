...
public interface DeleteOneEntityFieldsUsingOptionalCompletableFutureRepository {

    @Delete("DELETE FROM ${table} WHERE id=? RETURNING *")
    CompletableFuture<Optional<Account>> delete01(Long id);

    @Delete("DELETE FROM ${table} WHERE id=? RETURNING first_name, last_name")
    CompletableFuture<Optional<EntityFieldMap>> delete02(Long id);

    @Delete("DELETE FROM ${table} WHERE id=? RETURNING first_name, last_name")
    CompletableFuture<Optional<EntityFieldList>> delete03(Long id);
}
