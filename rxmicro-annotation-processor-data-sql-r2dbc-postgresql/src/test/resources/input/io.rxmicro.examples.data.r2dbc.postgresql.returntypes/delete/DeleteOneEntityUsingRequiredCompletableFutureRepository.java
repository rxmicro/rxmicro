...
public interface DeleteOneEntityUsingRequiredCompletableFutureRepository {
    ...
    CompletableFuture<Account> delete04(Account account);

    @Delete("DELETE FROM ${table} WHERE ${by-id-filter} RETURNING *")
    CompletableFuture<EntityFieldMap> delete05(Account account);

    @Delete("DELETE FROM ${table} WHERE ${by-id-filter} RETURNING *")
    CompletableFuture<EntityFieldList> delete06(Account account);
}
