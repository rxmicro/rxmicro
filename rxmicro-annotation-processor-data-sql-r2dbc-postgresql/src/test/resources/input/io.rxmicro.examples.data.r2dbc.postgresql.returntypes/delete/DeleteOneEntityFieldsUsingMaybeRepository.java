...
public interface DeleteOneEntityFieldsUsingMaybeRepository {

    @Delete("DELETE FROM ${table} WHERE id=? RETURNING *")
    Maybe<Account> delete01(Long id);

    @Delete("DELETE FROM ${table} WHERE id=? RETURNING first_name, last_name")
    Maybe<EntityFieldMap> delete02(Long id);

    @Delete("DELETE FROM ${table} WHERE id=? RETURNING first_name, last_name")
    Maybe<EntityFieldList> delete03(Long id);
}
