...
public interface SelectManyUsingCompletionStageListRepository {

    @Select("SELECT first_name, last_name FROM ${table} ORDER BY id")
    CompletionStage<List<Account>> findAll01();

    @Select("SELECT first_name, last_name FROM ${table} ORDER BY id")
    CompletionStage<List<EntityFieldMap>> findAll02();

    @Select("SELECT first_name, last_name FROM ${table} ORDER BY id")
    CompletionStage<List<EntityFieldList>> findAll03();

    @Select("SELECT email FROM ${table} ORDER BY id")
    CompletionStage<List<String>> findAll04();

    @Select("SELECT DISTINCT role::text FROM ${table} ORDER BY role")
    CompletionStage<List<Role>> findAll05();

    @Select("SELECT DISTINCT balance FROM ${table}")
    CompletionStage<List<BigDecimal>> findAll06();
}
