...
final class JsonDataHelper {
    ...
    static String getJsonStringFromClasspathResource(final String classpathResource) {
        try (InputStream is = JsonWriteIntegrationTest.class.getClassLoader().getResourceAsStream(classpathResource)) {
            try (InputStreamReader isr = new InputStreamReader(require(is), UTF_8);
